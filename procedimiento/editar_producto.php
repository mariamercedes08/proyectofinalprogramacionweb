<?php
	...
	if (!empty($_POST))
	{
		...
		{
			$alert='<p class="msg_error">Todos los campos son obligatorios y no negativos</p>';
		}else{
            ...
            if (($nombre_foto != '' && ($_POST['foto_actual'] != 'img_producto.png')) || ($_POST['foto_actual'] != $_POST['foto_remove'] )) {
              unlink('img/uploads/'.$_POST['foto_actual']);
						...
						if ($nombre_foto != '') {
							...
							$alert='<p class="msg_save">Producto actualizado correcto</p>';
					}else{
							$alert='<p class="msg_error">no se pudo Actualizar Producto</p>';
					}
			}
    }
    //validar producto
    if (empty($_REQUEST['id'])) {
        ...
        if ($result_producto > 0) {
          ...
          if ($data_producto['foto'] != 'img_producto.png') {
            $classRemove = '';
            $foto = '<img id="img" src="img/uploads/'.$data_producto['foto'].'">';
          }
        }else {
            header("location: lista_producto.php");
        }
